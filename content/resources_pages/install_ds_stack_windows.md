...
> **WebPDF is not available on Windows.** JupyterLab's
> `File > Save and Export Notebook As... > WebPDF` drives a headless browser in a
> way Windows does not support in the Python version MDS uses, so it fails there.
> Your setup check will show that line as FAILED, and there is nothing to fix.
> On Windows, **Typst is the route for documents containing emoji or other
> symbols** — `quarto render report.ipynb --to typst`, which reads notebooks
> directly.
...
WebPDF prints through a headless browser, and the library that drives it cannot
start subprocesses on Windows in the Python version MDS uses. Nothing on your
machine is wrong and there is nothing to fix; see
[Which PDF route to use](#which-pdf-route-to-use) for what to do when you need a
PDF containing emoji.
